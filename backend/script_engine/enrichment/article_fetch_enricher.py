...
def fetch_url_text(url: str) -> str:
    try:
        ...
        garbage_terms = ["cookie", "privacy", "terms", "consent"]
        if any(term in text.lower() for term in garbage_terms):
            return ""
...
def build_enrichment(show_structure: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for act in show_structure.get("acts", []):
        for story in act.get("stories", []):
            ...
            summary_fallback = clean(story.get("summary"))
